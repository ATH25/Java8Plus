...
- Pattern Matching for switch (Java 17/21+)
  - Exhaustive and nested switch patterns on records, enums, and classic types
...
- Virtual Threads (Java 21+)
  ...
  - Enable the creation of millions of concurrent threads for tasks like web requests, I/O, and background jobs—eliminating traditional “thread pool” bottlenecks.
  - Dramatically simplify scalable, maintainable concurrent programming by enabling per-task thread creation with almost no overhead.
  ...
  - Example exercises:
    ...
    * Comparing classic platform threads and virtual threads (`ThreadComparisonDemo.java`)
    * Managing thread lifecycle with join() and state inspection (`ThreadJoinAndStatusDemo.java`)
    * Handling task results and exceptions in virtual threads (upcoming)
  - Best practices:
    * Use virtual threads for high-concurrency, I/O-bound workloads.
    * Prefer thread-per-task style for simplicity; avoid excessive thread-local state.
    * Name your virtual threads explicitly when needed using `Thread.ofVirtual().name(...)` for easier debugging.
  - Comparison: Platform vs. Virtual Threads

    | Feature                     | Platform Thread                   | Virtual Thread                        |
    |----------------------------|-----------------------------------|---------------------------------------|
    | Backed By                  | OS thread                         | JVM-managed lightweight thread        |
    | Memory & Resource Overhead | High                              | Very Low                              |
    | Creation Cost              | Expensive                         | Cheap (millions possible)             |
    | Scheduling                 | OS scheduler                      | Java Scheduler (continuation-based)   |
    | Blocking Calls             | Blocks kernel thread              | Does not block carrier thread         |
